...
<mujoco>
  ...
  <worldbody>
    ...
    <body name="ball" pos="0 0 1.15">
      ...
      <geom type="sphere" mass="0.05" size="0.025" solref="0.01 0.1" condim="4" priority="1"/>
    </body>
  </worldbody>

  <actuator>
    <motor name="z_motor" joint="free_joint" gear="0 0 1 0 0 0" ctrlrange="-10 10"/>
  </actuator>
</mujoco>
"""
...
if __name__ == "__main__":
    ...
    opt_timestep = 0.0005  # timestep (s)
    ...
    Fz_acc_control = -5  # force during acceleration phase (downward)
    Fz_decc_control = 10.0   # force during deceleration phase (upward)

    # Desired speed magnitudes (m/s) for bang–bang
    v_des = 10.0  # speed to reach before deceleration (magnitude)
    ...
    max_distance = np.abs(0.5 * a_acc_net * opt_timestep**2) + v_des * opt_timestep
    ...
    try:
        ...
        while env.viewer.is_running() and env.data.time < 2.0:
            ...
            if phase == "bangbang":
                ...
                in_acceleration = current_z - table_z - ball_radius > decel_distance
                if in_acceleration:
            ...
            z_positions.append(current_z-table_z-ball_radius)
    ...
    # Z-position plot
    axs[1].plot(times, z_positions)
    axs[1].set_title('Ball Height (z)')
    axs[1].set_ylabel('Z Position (m)')
    axs[1].grid(True)
    
    # Velocity plot
    axs[2].plot(times, velocities, label='Vertical Velocity')
    ...
    # Control input plot
    axs[3].plot(times, ctrl_inputs)
    axs[3].set_title('Control Inputs')
    axs[3].set_ylabel('Force (N)')
    axs[3].set_xlabel('Time (s)')
    axs[3].grid(True)
     # Draw horizontal lines for Fz_acc and Fz_decc
